...
tags["lyn:hpn"] = function(unit)
	local name, realm = UnitName(unit)
	
	name = utf8sub(name, 13, true)
	
	if not UnitIsConnected(unit) then
		return "|cff333333"..name.."|r"
	elseif UnitIsGhost(unit) then
		return "|cff999999"..name.."|r"
	elseif UnitIsDead(unit) then
		return "|cff555555"..name.."|r"
	end
		
	local _, class = UnitClass(unit)
	local min, max = UnitHealth(unit), UnitHealthMax(unit)
	local percent = math.floor((min / max) * 100+0.5)
	if percent > 95 and class ~= nil then
		return hex(oUF.colors.class[class]) .. name
	else
		return hex(oUF.ColorGradient(min, max, 1,0,0, 1,1,0, 0,1,0)) .. name
	end
end
tagevents["lyn:hpn"] = "UNIT_HEALTH UNIT_MAXHEALTH UNIT_CONNECTION UNIT_NAME_UPDATE"
